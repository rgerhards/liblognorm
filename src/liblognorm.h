...
#define LN_CTXOPT_ADD_EXPECTED_NEXT	0x80 /**< add bounded expected-next diagnostics on parse failure;
					      disables TurboVM for this context and emits a warning
					      through the error callback when both are requested */
/**
 * Set options on ctx.
 *
