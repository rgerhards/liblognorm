/* rulebase.c -- code for ln_rb objects.
 *
...
#include "parser.h"
void
ln_rbClose(ln_ctx __attribute((unused)) ctx, struct ln_rbRepos *repo)
{
	if(repo->fp != NULL)
		fclose(repo->fp);
	free(repo);
 ...
 */
struct ln_rb*
ln_rbCreate(ln_ctx __attribute__((unused)) ctx)
{
	struct ln_rb* samp;

	if((samp = calloc(1, sizeof(struct ln_rb))) == NULL)
		goto done;
...
void
ln_rbFree(ln_ctx __attribute__((unused)) ctx, struct ln_rb *samp)
{
	free(samp);
}
...
#define REC_RULE 0
#define REC_TYPEDEF 1
#define REC_COMMENT 2


static int
getRecType(ln_ctx ctx, FILE *rb, int *const __restrict__ recType)
{
	int r = 0;
done:
	return r;
...
int
ln_rbLoad(ln_ctx ctx, const char *file)
{
	int r = 0;
	...
	CHECK_CTX;
	if((rb = fopen(name, "r")) == NULL) ERR_ABORT;
	while(1) { /* broken inside */
		CHKR(getRecType(ctx, rb, &rectype));
	}
...
struct ln_rbRepos*
ln_rbOpen(ln_ctx __attribute((unused)) ctx, const char *name)
{
	struct ln_rbRepos *repo = NULL;
	FILE *fp;

	if((fp = fopen(name, "r")) == NULL)
		goto done;

	if((repo = calloc(1, sizeof(struct ln_rbRepos))) == NULL) {
		fclose(fp);
		goto done;
	}
